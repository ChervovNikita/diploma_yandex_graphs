...
def make_model(model_name, num_layers, input_dim, num_targets, device):
    return Model(
...
@torch.no_grad()
def evaluate_from_logits(logits, data, mask, is_binary):
    return compute_metrics(logits, data.y, mask, is_binary)
...
def train_single_seed(model_name, num_layers, model_seed, data, num_targets, is_binary, device, save_dir,
    ...
    ckpt_path = os.path.join(save_dir, f'{model_name}_{num_layers}L_split{split_idx}_mseed{model_seed}.pt')
    ...
    for step in range(1, NUM_STEPS + 1):
        ...
        if step % 50 == 0 or step == 1:
            print(f'  [mseed={model_seed}] Step {step:4d} | loss {train_loss:.4f} | val_acc {val_metrics["acc"]:.4f}')

        if steps_without_improvement >= PATIENCE * 100:
            print(f'  [mseed={model_seed}] Early stopping at step {step}')
            break

    print(f'  [mseed={model_seed}] Best val_acc={best_val_acc:.4f} @ step {best_step}')
...
def train_ensemble(model_name, num_layers, split_idx, data, train_masks, val_masks, test_masks,
    ...
    val_m = evaluate_from_logits(avg_logits, data, data.val_mask, is_binary)
    test_m = evaluate_from_logits(avg_logits, data, data.test_mask, is_binary)
    train_m = evaluate_from_logits(avg_logits, data, data.train_mask, is_binary)
    ...
    print(f'  Ensemble train_acc={train_m["acc"]:.4f}  val_acc={val_m["acc"]:.4f}')
    print(f'  Ensemble test_acc={test_m["acc"]:.4f}  test_loss={test_m["loss"]:.4f}')
    if not is_binary:
...
def main():
    ...
    fieldnames = ['dataset', 'model', 'num_layers', 'split', 'num_model_seeds', 'best_steps',
                  'individual_val_accs', 'train_acc', 'val_acc',
