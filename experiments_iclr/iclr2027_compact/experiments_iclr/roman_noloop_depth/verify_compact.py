"""Check all 12 decision records and validation-selection traces in the compact archive."""
...
for name in ('roman_noloop_depth.py','roman_noloop_depth_protocol.md','models.py'):
    need(sha(ROOT/name)==frozen[name],f'Frozen source changed: {name}')
count=0
for dataset in ('roman',):
    for depth in (2,5):
        base=ROOT/'results'/dataset/f'depth{depth}'
        manifest=json.loads((base/'source_manifest.json').read_text())
        need(manifest['protocol']=='roman_noloop_depth_v1' and manifest['configuration']['layers']==depth,'manifest protocol/depth')
        ...
        for seed in (0,1,2):
            ...
            for arm in ('tied','untied_propagation'):
                ...
                need(row['protocol']=='roman_noloop_depth_v1' and row['dataset']==dataset and row['depth']==depth and row['optimization_seed']==seed and row['arm']==arm,'run identity')
                ...
                count+=1
need(count==12,'incomplete compact grid')
print('PASS_COMPACT_12_ROMAN_NOLOOP_DEPTH_CELLS')
