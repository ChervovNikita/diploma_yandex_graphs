"""Independent artifact and selected-checkpoint replay for cross-graph study.
...
import sharing_crossgraph as study
...
def homophily(dataset):
    loader = study.load_wikics if dataset == "wikics" else study.load_actor
    _, labels, _, edges, _, _, _ = loader()
    source, target = edges
...
def audit_dataset(dataset: str, device: torch.device):
    ...
    for seed in study.SEEDS:
        ...
        for arm in study.ARMS:
            ...
            require(row["protocol"] == "sharing_boundary_crossgraph_split0_ogb300_v1" and
