"""Fixed-split cross-graph sharing-boundary diagnostic.

Runs WikiCS and Actor official split 0 with optimizer seeds 0/1/2. The two
...
DATA_FILES = {
    ...
    "actor": ("data/actor/out1_node_feature_label.txt",
              "data/actor/out1_graph_edges.txt",
              "data/actor/film_split_0.6_0.2_0.npz"),
}
SOURCE_FILES = ("sharing_crossgraph.py", "sharing_crossgraph_protocol.md",
                "verify_sharing_crossgraph.py", "models.py")
...
def load_graph(dataset: str, device: torch.device):
    if dataset == "wikics":
        x, y, raw_edge, edge, train, valid, test = load_wikics()
    elif dataset == "actor":
        x, y, raw_edge, edge, train, valid, test = load_actor()
    else:
...
def expected_manifest(dataset: str, descriptor: dict) -> dict:
    ...
    return {
        "protocol": "sharing_boundary_crossgraph_split0_ogb300_v1",
        "dataset": dataset,
        "split": 0,
