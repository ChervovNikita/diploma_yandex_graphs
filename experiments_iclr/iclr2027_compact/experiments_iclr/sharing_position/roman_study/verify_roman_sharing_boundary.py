"""Independent artifact and selected-checkpoint replay for sharing boundary v1.

Writes completion_audit.json only after all 20 arms on one dataset pass.
...
import roman_sharing_boundary as study
...
def audit_dataset(dataset: str, device: torch.device):
    ...
    require(seed_dirs == {f"seed{s}" for s in study.SEEDS},
            f"Expected exactly five seed directories, found {seed_dirs}")
    records = []
    for seed in study.SEEDS:
        ...
        for arm in study.ARMS:
            ...
            require(row["protocol"] == "roman_sharing_boundary_mask0_ogb300_v1" and
