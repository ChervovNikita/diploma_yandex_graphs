...
def full_label_and_test_index(dataset: str, frozen: dict):
    loader = {"cora": study.load_cora, "wikics": study.load_wikics,
              "actor": study.load_actor,
              "chameleon_filtered": study.load_filtered}[dataset]
    _, labels, _, _, _, _, test_mask = loader()
    ...
    return labels.numpy(), indices.numpy()
...
def main() -> None:
    ...
    graphs, cells = {}, {}
    for dataset in DATASETS:
        labels, test_index = full_label_and_test_index(dataset, frozen)
        require(labels.dtype == test_index.dtype == np.int64 and
                int(labels.max()) < np.iinfo(np.int16).max and
                int(labels.min()) >= 0 and len(set(test_index.tolist())) == len(test_index),
        ...
        graphs[dataset] = {
            ...
            "test_nodes": len(test_index), "classes": int(labels.max()) + 1,
        }
