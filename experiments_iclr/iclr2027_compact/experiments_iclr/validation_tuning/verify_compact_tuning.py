"""Public compact audit of the 432-cell selection and hard class scores.
...
def audit_selection(lock: dict, freeze_sha: str) -> set[str]:
    ...
    require(seen == set(lock["cells"]) == actual and len(seen) == 432,
...
def audit_hard_scores(lock: dict, freeze_sha: str) -> int:
    ...
    require(final["protocol"] == "validation_tuning_sensitivity_v1" and
    ...
    require(strict["script_sha256"] ==
            ...
            all(row["valid_decision_mismatches"] == 0 and
                row["test_decision_mismatches"] == 0
    ...
    for dataset in DATASETS:
        ...
        require(labels.dtype == indices.dtype == np.int64 and
        ...
        references[dataset] = (labels, indices)

    expected = set()
    for dataset in DATASETS:
        labels, indices = references[dataset]
