"""Prospectively fixed external SAGE controls for the GNNM ICLR study.

Runs WikiCS and Actor on their published split 0, with optimization seeds
0/1/2.  The architecture and optimization schedule match the separate
ogbn-arxiv 300-epoch study.  Test labels are scored only after a validation
selected checkpoint is restored.  All files remain below this script's root.
"""
...
SOURCE_FILES = ("external_depth_grid.py", "external_depth_protocol.md", "models.py")
...
def load_graph(dataset: str, device: torch.device):
    x, y, raw_edge, edge, train, valid, test = (
        load_wikics() if dataset == "wikics" else load_actor()
    )
    n = len(y)
...
def expected_manifest(dataset: str, descriptor: dict) -> dict:
    ...
    return {
        "protocol": "external_sage_depth_grid_v1",
        "dataset": dataset,
        "split": 0,
