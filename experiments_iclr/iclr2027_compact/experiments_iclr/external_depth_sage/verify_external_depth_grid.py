"""Independent artifact and selected-checkpoint replay for external SAGE v3.

Writes completion_audit.json only after all nine arms on one dataset pass.
...
import external_depth_grid as study
...
def homophily(dataset):
    _, labels, _, edges, _, _, _ = (
        study.load_wikics() if dataset == "wikics" else study.load_actor()
    )
    source, target = edges
...
def audit_dataset(dataset: str, device: torch.device):
    ...
    for seed in study.SEEDS:
        ...
        for arm in study.ARMS:
            ...
            require(row["protocol"] == "external_sage_depth_grid_v1" and
                    row["dataset"] == dataset and row["depth"] == study.DEPTH and row["published_split"] == 0 and
