"""Frozen depth controls on the official filtered Chameleon graph.

Uses Platonov et al. filtered Chameleon mask 0 and the prior SAGE recipe.
Every depth/arm/seed cell is mandatory; test labels are scored only after
restoring the validation-selected checkpoint. All files remain here.
"""
...
DATA_FILES = {"chameleon_filtered": ("data/chameleon_filtered.npz",)}
...
def load_public(dataset: str):
    if dataset != "chameleon_filtered":
        raise ValueError(dataset)
    with np.load(ROOT / "data/chameleon_filtered.npz", allow_pickle=False) as f:
        if set(f.files) != {"node_features", "node_labels", "edges",
                            "train_masks", "val_masks", "test_masks"}:
            raise RuntimeError("Filtered Chameleon archive schema changed")
        x = torch.from_numpy(f["node_features"].copy()).float()
        y = torch.from_numpy(f["node_labels"].copy()).long()
        raw_edge = torch.from_numpy(f["edges"].T.copy()).long().contiguous()
        masks = [torch.from_numpy(f[key][0].copy()).bool() for key in
                 ("train_masks", "val_masks", "test_masks")]
        if any(f[key].shape != (10, len(y)) for key in
               ("train_masks", "val_masks", "test_masks")):
            raise RuntimeError("Filtered Chameleon mask count changed")
    edge = to_undirected(coalesce(raw_edge, num_nodes=len(y)), num_nodes=len(y))
    return x, y, raw_edge, edge, *masks
...
def expected_manifest(dataset: str, descriptor: dict) -> dict:
    ...
    return {
        "protocol": "filtered_chameleon_sage_depth_v1",
        "dataset": dataset,
        "split": "Platonov filtered 0",
...
def train_one(dataset: str, bundle, source_sha: str, seed: int,
    ...
    row = {
        "protocol": "filtered_chameleon_sage_depth_v1",
        "dataset": dataset,
        "published_split": "Platonov filtered 0",
