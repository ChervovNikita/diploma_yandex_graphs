"""Frozen official filtered-Chameleon sharing-position holdout test.

Runs the Platonov filtered graph's mask 0 with four equal-budget arms and
seeds 0/1/2. The two partial-sharing arms put one private SAGE block first
or last with equal parameter counts. Test labels are scored only after
restoring the validation-selected checkpoint.
"""
...
def expected_manifest(dataset: str, descriptor: dict) -> dict:
    ...
    return {
        "protocol": "filtered_chameleon_sharing_position_v1",
        "dataset": dataset,
        "split": "Platonov filtered 0",
...
def train_one(dataset: str, bundle, source_sha: str, seed: int,
    ...
    row = {
        "protocol": "filtered_chameleon_sharing_position_v1",
        "dataset": dataset, "published_split": "Platonov filtered 0",
