"""Post-audit compact projection of the frozen normalized-Cora matched54 study."""
...
def main():
    ...
    bundle, _ = primary.load_graph("cora_normalized", torch.device("cpu"), include_test=True)
    ...
    for name in ("FROZEN_CORA_MATCHED54.json", "PREFLIGHT_CUDA.json",
                 "VALIDATION_SELECTION_LOCK.json", "FINAL_SCORE_AUDIT.json", "SUMMARY.json"):
        copy(OUT / name, STAGE / name)
    for name in ("cora_matched54.py", "verify_cora_matched54.py",
                 "CORA_MATCHED54_PROTOCOL.md", "CORA_MATCHED54_WIDTH_SCAN.json",
                 "build_cora_matched54_compact.py", "verify_cora_matched54_compact.py"):
    ...
    readme = ("# Normalized-Cora parameter-matched post hoc evidence\n\n"
              ...
              "Run `python3 source/verify_cora_matched54_compact.py .` with NumPy. "
    ...
    with tarfile.open(OUT / "CORA_MATCHED54_COMPACT.tar.xz", "w:xz", preset=6) as tar:
        tar.add(STAGE, arcname="cora_matched54")
    print(json.dumps({"stage": str(STAGE), "archive": str(OUT / "CORA_MATCHED54_COMPACT.tar.xz"),
                      "archive_bytes": (OUT / "CORA_MATCHED54_COMPACT.tar.xz").stat().st_size,
                      "archive_sha256": study.sha(OUT / "CORA_MATCHED54_COMPACT.tar.xz")}), flush=True)
