"""Frozen post hoc normalized-Cora parameter-matched 54-cell comparator study."""
...
OUT = ROOT / "cora_matched54_results"
FREEZE = OUT / "FROZEN_CORA_MATCHED54.json"
PROTOCOL = "cora_matched54_posthoc_v1"
ARMS = ("base", "ens", "private_last")
SEEDS = (0, 1, 2)
CANDIDATES = primary.CANDIDATES
DEFAULT = primary.DEFAULT
WIDTHS = {"base": 184, "ens": 70, "private_last": 128}
PARAMETER_COUNTS = {"base": 605919, "ens": 602868, "private_last": 604700}
TARGET = 604700
SOURCES = ("cora_matched54.py", "verify_cora_matched54.py",
           "CORA_MATCHED54_PROTOCOL.md", "CORA_MATCHED54_WIDTH_SCAN.json")
...
def key(arm, lr, wd, seed):
    return f"cora_normalized/{arm}/{primary.candidate_name(lr, wd)}/seed{seed}"
...
def width_scan():
    ...
    require(base_rows[0] == (1219, 184, 605919) and
            ens_rows[0] == (1832, 70, 602868), "Width scan changed")
    return {"protocol": PROTOCOL, "criterion": "minimum absolute stored parameter-count gap, then lower width; no outcomes used",
            ...
            "base_formula": "10*w*w+1453*w+7", "base_range": [32, 512],
            "base_selected_width": 184, "base_parameters": 605919, "base_gap": 1219,
            "ens_formula": "4*(10*w*w+1453*w+7)", "ens_range": [32, 128],
            "ens_selected_width": 70, "ens_parameters": 602868, "ens_gap": 1832}


def expected_freeze():
    require(primary.WIDTH == 128, "Original width must be restored")
    require(json.loads((ROOT / "CORA_MATCHED54_WIDTH_SCAN.json").read_text()) == width_scan(),
            "Parameter-only width scan differs")
    original = json.loads((ROOT / "FROZEN_STUDY.json").read_text())
    require(original["graphs"]["cora_normalized"]["descriptor"]["features"] == 1433 and
            original["graphs"]["cora_normalized"]["descriptor"]["classes"] == 7,
            "Original Cora feature/class dimensions differ")
    return {"protocol": PROTOCOL, "posthoc_after_primary_outcomes": True,
            ...
            "graph": original["graphs"]["cora_normalized"],
            "matrix": {"dataset": "cora_normalized", "arms": list(ARMS), "widths": WIDTHS,
                       ...
                       "complete_audit_cutoff_utc": "2026-09-26T08:25:00Z"}}
        ...
        for candidate_index, (lr, wd) in enumerate(CANDIDATES):
            for seed in SEEDS:
                if (candidate_index * len(SEEDS) + seed) % 2 != partition:
                    continue
                primary.train_one("cora_normalized", bundle, freeze_sha, arm, lr, wd, seed, device)
            ...
            for lr, wd in dict.fromkeys((selected, DEFAULT)):
                for seed in SEEDS:
                    ...
                    primary.write_json(output / "score.json", {
                        ...
                        "dataset": "cora_normalized", "arm": arm, "width": WIDTHS[arm],
                        "lr": lr, "weight_decay": wd, "seed": seed,
