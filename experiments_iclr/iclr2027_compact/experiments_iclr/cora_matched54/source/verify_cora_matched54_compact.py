"""Standalone NumPy replay of compact normalized-Cora matched54 records."""
...
def key(arm, lr, wd, seed):
    return f"cora_normalized/{arm}/lr{lr:g}_wd{wd:g}/seed{seed}"
...
def main(root):
    ...
    freeze = json.loads((root / "FROZEN_CORA_MATCHED54.json").read_text())
    ...
    require(freeze["protocol"] == lock["protocol"] == audit["protocol"] == manifest["protocol"] and
            sha(root / "FROZEN_CORA_MATCHED54.json") == manifest["freeze_sha256"] and
        ...
        for lr, wd in CANDIDATES:
            ...
            for seed in SEEDS:
                ...
                require(result["freeze_sha256"] == manifest["freeze_sha256"] and
                        result["dataset"] == "cora_normalized" and result["arm"] == arm and
