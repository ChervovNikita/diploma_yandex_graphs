"""Independent validation-only audit and post-lock replay for the tuning study.
...
def audit_and_lock():
    ...
    require(len(cells) == 432 and sum(len(v) for v in selections.values()) == 24,
            "Full validation matrix incomplete")
    lock = {"protocol": "validation_tuning_sensitivity_v1", "freeze_sha256": freeze_sha,
    ...
    print(json.dumps({"lock_sha256": sha(lock_path), "cells": len(cells),
                      "groups": 24, "partial_family_choice": partial_choice}), flush=True)
...
def audit_scores(device: torch.device):
    ...
    require(lock["freeze_sha256"] == freeze_sha and len(lock["cells"]) == 432,
            "Invalid global selection lock")
    lock_sha = sha(lock_path)
    expected_keys = set()
    report = {"protocol": "validation_tuning_sensitivity_v1", "freeze_sha256": freeze_sha,
