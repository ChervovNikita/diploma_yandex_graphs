"""Audit conditional seed intervals for the post hoc Cora storage controls."""
...
STAGE = HERE.parent / "cora_matched54"
...
def selected_scores(arm: str, lock: dict, audit: dict, manifest: dict) -> tuple[list[float], list[float]]:
    ...
    scores = []
    for seed in (0, 1, 2):
        key = f"cora_normalized/{arm}/lr{lr:g}_wd{wd:g}/seed{seed}"
...
def calculate() -> dict:
    ...
    need(manifest["protocol"] == "cora_matched54_posthoc_v1" and
         sha(lock_path) == manifest["validation_lock_sha256"] and
         sha(audit_path) == manifest["score_audit_sha256"],
         "Matched Cora manifest/lock/audit differs")
    ...
    need(audit["validation_selection_lock_sha256"] == sha(lock_path),
         "Matched Cora score audit lock differs")
    values = {}
    candidates = {}
    ...
    for comparator in ("ens", "base"):
        ...
        rows.append({"graph": "cora_normalized", "partial_arm": "private_last",
...
def main() -> None:
    need(len(sys.argv) == 1 or sys.argv == [sys.argv[0], "--check"],
         "Usage: python matched_cora_seed_intervals.py [--check]")
    report = calculate()
    ...
    for name, content in (("MATCHED_CORA_CONDITIONAL_INTERVALS.json", json_text),
                          ("MATCHED_CORA_CONDITIONAL_INTERVALS.csv", stream.getvalue())):
        path = HERE / name
        if "--check" in sys.argv:
            need(path.read_bytes() == content.encode(),
                 f"Packaged Cora interval output differs: {name}")
        else:
