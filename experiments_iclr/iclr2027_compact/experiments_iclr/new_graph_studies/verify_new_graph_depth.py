...
Writes completion_audit.json only after all six arms at one depth pass.
...
import new_graph_depth as study
...
def homophily(dataset):
    _, labels, _, edges, _, _, _ = study.load_public(dataset)
    source, target = edges
...
def audit_dataset(dataset: str, device: torch.device):
    root = study.ROOT / "results" / dataset / f"depth{study.DEPTH}"
    ...
    for seed in study.SEEDS:
        ...
        for arm in study.ARMS:
            ...
            require(row["protocol"] == "new_graph_sage_depth_v1" and
                    row["dataset"] == dataset and row["depth"] == study.DEPTH and
                    row["published_split"] == ("Planetoid public" if dataset == "cora" else "Geom-GCN 0") and
    ...
    result = {
        ...
        "dataset": dataset, "depth": study.DEPTH,
...
def main():
    ...
    parser.add_argument("--depth", required=True, type=int, choices=(2, 5))
