"""Fixed public-graph depth controls for the GNNM ICLR study.

Runs Planetoid Cora public split and Geom-GCN Chameleon split 0 using the
previously fixed SAGE depth recipe. Every depth/arm/seed cell is mandatory.
Test labels are scored only after restoring the validation-selected checkpoint.
All files remain below this script's root.
"""
...
SOURCE_FILES = ("new_graph_depth.py", "new_graph_depth_protocol.md",
                "verify_new_graph_depth.py", "models.py")
...
def raw_dataset_files(dataset: str) -> tuple[str, ...]:
    raw = ROOT / "data" / dataset
    paths = sorted(p.relative_to(ROOT).as_posix() for p in raw.rglob("*")
                   if p.is_file() and "raw" in p.relative_to(raw).parts)
    if not paths:
        raise RuntimeError(f"No downloaded raw public dataset files: {dataset}")
    return tuple(paths)


def load_public(dataset: str):
    if dataset == "cora":
        graph = Planetoid(root=str(ROOT / "data/cora"), name="Cora",
                          split="public")[0]
        train, valid, test = graph.train_mask, graph.val_mask, graph.test_mask
    elif dataset == "chameleon":
        graph = WikipediaNetwork(root=str(ROOT / "data/chameleon"),
                                 name="chameleon", geom_gcn_preprocess=True)[0]
        if any(mask.ndim != 2 or mask.size(1) != 10 for mask in
               (graph.train_mask, graph.val_mask, graph.test_mask)):
            raise RuntimeError("Chameleon Geom-GCN mask count changed")
        train, valid, test = (graph.train_mask[:, 0], graph.val_mask[:, 0],
                              graph.test_mask[:, 0])
    else:
        raise ValueError(dataset)
    x = graph.x.float().contiguous()
    y = graph.y.long().flatten().contiguous()
    raw_edge = graph.edge_index.long().contiguous()
    edge = to_undirected(coalesce(raw_edge, num_nodes=len(y)), num_nodes=len(y))
    return x, y, raw_edge, edge, train.bool(), valid.bool(), test.bool()


def load_graph(dataset: str, device: torch.device):
    x, y, raw_edge, edge, train, valid, test = load_public(dataset)
    n = len(y)
...
def expected_manifest(dataset: str, descriptor: dict) -> dict:
    paths = SOURCE_FILES + raw_dataset_files(dataset)
    return {
        "protocol": "new_graph_sage_depth_v1",
        "dataset": dataset,
        "split": "Planetoid public" if dataset == "cora" else "Geom-GCN 0",
...
def verify_manifest(dataset: str, descriptor: dict) -> str:
    ...
    path = ROOT / "results" / dataset / f"depth{DEPTH}" / "source_manifest.json"
    if path.exists():
...
def initial_audit(model, canonical_sha, bundle, seed: int, arm: str,
    ...
    if arm != "tied":
        paired = ROOT / "results" / dataset_arg / f"depth{DEPTH}" / f"seed{seed}" / "tied"
...
def train_one(dataset: str, bundle, source_sha: str, seed: int,
    ...
    final = ROOT / "results" / dataset / f"depth{DEPTH}" / f"seed{seed}" / arm
    if final.exists():
    ...
    row = {
        "protocol": "new_graph_sage_depth_v1",
        "dataset": dataset,
        "published_split": "Planetoid public" if dataset == "cora" else "Geom-GCN 0",
        "optimization_seed": seed, "arm": arm, "depth": DEPTH,
...
def main():
    ...
    parser.add_argument("--depth", type=int, choices=(2, 5), required=True)
