"""Frozen Cora/legacy-Chameleon sharing-position holdout test.

Runs the Planetoid Cora public split and Geom-GCN Chameleon split 0 with all
four equal-budget arms and seeds 0/1/2. The two partial-sharing arms put one
private SAGE block first or last with equal parameter counts. Test labels are
scored only after restoring the validation-selected checkpoint.
"""
...
import new_graph_depth as depth_study
...
DATA_FILES = {"cora": (), "chameleon": ()}
SOURCE_FILES = (
    "new_graph_sharing.py", "new_graph_sharing_protocol.md",
    "verify_new_graph_sharing.py", "new_graph_depth.py",
    "new_graph_depth_protocol.md", "models.py",
    "POSITION_PREDICTION_FREEZE.md", "PRETRAIN_FREEZE.json",
)
...
def load_graph(dataset: str, device: torch.device):
    ...
    return depth_study.load_graph(dataset, device)



def expected_manifest(dataset: str, descriptor: dict) -> dict:
    paths = SOURCE_FILES + depth_study.raw_dataset_files(dataset)
    return {
        "protocol": "new_graph_sharing_position_v1",
        "dataset": dataset,
        "split": "Planetoid public" if dataset == "cora" else "Geom-GCN 0",
...
def verify_manifest(dataset: str, descriptor: dict) -> str:
    ...
    path = ROOT / "position_results" / dataset / "source_manifest.json"
    if path.exists():
...
def initial_audit(model, canonical_sha, bundle, seed: int, arm: str,
    ...
    if arm != "tied":
        paired = ROOT / "position_results" / dataset_arg / f"seed{seed}" / "tied"
...
def train_one(dataset: str, bundle, source_sha: str, seed: int,
    ...
    final = ROOT / "position_results" / dataset / f"seed{seed}" / arm
    if final.exists():
    ...
    row = {
        "protocol": "new_graph_sharing_position_v1",
        "dataset": dataset, "published_split": "Planetoid public" if dataset == "cora" else "Geom-GCN 0",
