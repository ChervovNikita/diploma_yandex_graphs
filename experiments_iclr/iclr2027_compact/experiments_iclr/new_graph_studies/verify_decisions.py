"""NumPy-only audit of the complete 48-arm Cora/Chameleon decision stage."""
...
def group_check(base: Path, graph: str, arms: tuple[str, ...], seed_count: int,
    ...
    for relative, digest in manifest["source_sha256"].items():
        if relative.startswith("data/"):
            require(len(digest) == 64, f"Public raw-data hash syntax: {relative}")
        else:
...
def main() -> None:
    ...
    require(len(derivations) == 48, "Expected 48 selected decision records")
    depth_freeze = read(ROOT / "PRETRAIN_FREEZE.json")
    position_freeze = read(ROOT / "POSITION_PRETRAIN_FREEZE.json")
    require(depth_freeze["status"] == "FROZEN_BEFORE_TRAINING" and
            position_freeze["status"] ==
            "FROZEN_BEFORE_ANY_CORA_OR_CHAMELEON_TRAINING" and
            position_freeze["depth_pretrain_freeze_sha256"] ==
            sha(ROOT / "PRETRAIN_FREEZE.json") and
            position_freeze["position_prediction_sha256"] ==
            sha(ROOT / "POSITION_PREDICTION_FREEZE.md"),
    ...
    for graph in GRAPHS:
        ...
        for depth in (2, 5):
            ...
            expected = depth_freeze["studies"][graph]["manifests"][f"depth{depth}"]["sha256"]
            paired, descriptor = group_check(base, graph, DEPTH_ARMS, 3,
                                             "new_graph_sage_depth_v1", expected,
                                             derivations)
            require(descriptor == depth_freeze["studies"][graph]["data_descriptor"],
                    f"Frozen depth data: {graph}/{depth}")
            result[graph][f"depth{depth}_tied_minus_untied_pp"] = paired
        base = Path("position_results") / graph
        expected = position_freeze["studies"][graph]["source_manifest_sha256"]
        paired, descriptor = group_check(base, graph, POSITION_ARMS, 3,
                                         "new_graph_sharing_position_v1", expected,
                                         derivations)
        require(descriptor == position_freeze["studies"][graph]["data_descriptor"],
                f"Frozen position data: {graph}")
        result[graph]["private_first_minus_private_last_pp"] = paired
    ...
    print(json.dumps({"status": "NEW_GRAPH_48_ARM_DECISION_AUDIT_PASS",
                      "runs": 48, "paired_percentage_points": result}, sort_keys=True))
