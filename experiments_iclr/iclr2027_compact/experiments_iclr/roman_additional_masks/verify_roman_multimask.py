"""Independent 48-arm selected-checkpoint replay for Roman mask/depth extension."""
...
def audit_dataset(dataset: str, device: torch.device):
    root = study.result_root(dataset)
    ...
    for seed in study.SEEDS:
        ...
        for arm in study.ARMS:
            ...
            require(row["protocol"] == study.PROTOCOL and
                    row["dataset"] == dataset and row["published_split"] == study.MASK and
                    row["depth"] == study.DEPTH and
            ...
            require(study.hash_rng_state(random.getstate()) == init["python_rng_sha256"] and
                    study.hash_rng_state(np.random.get_state()) == init["numpy_rng_sha256"],
                    f"Reconstructed Python/NumPy RNG differs: {run}")
            ...
            require(checkpoint["epoch"] == row["selected_epoch"] and
                    checkpoint["seed"] == seed and checkpoint["arm"] == arm and
                    checkpoint["mask"] == study.MASK and checkpoint["depth"] == study.DEPTH,
...
def main():
    ...
    require(args.mask in (1, 2, 3, 4) and args.depth in (2, 5),
            "Frozen extension requires masks 1-4 and depths 2 or 5")
    study.MASK = args.mask
    study.DEPTH = args.depth
