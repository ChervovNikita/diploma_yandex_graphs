"""Post hoc Roman Empire mask/depth extension; see roman_multimask_protocol.md."""
...
SOURCE_FILES = ("roman_multimask.py", "verify_roman_multimask.py",
                "roman_multimask_orchestrator.py",
                "roman_multimask_protocol.md", "models.py")
PROTOCOL = "roman_multimask_no_added_loops_depth2_5_1000_v1"


def result_root(dataset: str) -> Path:
    return ROOT / "results" / dataset / f"mask{MASK}" / f"depth{DEPTH}"
...
def load_roman():
    ...
    with np.load(path, allow_pickle=False) as data:
        ...
        train = torch.from_numpy(data["train_masks"][MASK].copy()).bool()
        valid = torch.from_numpy(data["val_masks"][MASK].copy()).bool()
        test = torch.from_numpy(data["test_masks"][MASK].copy()).bool()
    edge = to_undirected(coalesce(raw_edge, num_nodes=len(y)), num_nodes=len(y))
    if edge.shape != (2, 65854) or bool((edge[0] == edge[1]).any()):
        raise RuntimeError("Expected 65,854 non-self directed entries")
    return x, y, raw_edge, edge, train, valid, test
...
def load_graph(dataset: str, device: torch.device):
    ...
    if tuple(int(indices[k].numel()) for k in ("train", "valid", "test")) != (11331, 5665, 5666):
        raise RuntimeError("Published partition size changed")
    descriptor = {
        "dataset": dataset,
        "split": MASK,
        "num_nodes": n,
...
def expected_manifest(dataset: str, descriptor: dict) -> dict:
    ...
    return {
        ...
        "arms": list(ARMS),
        "architecture": "original models.TABMModel SAGE with fixed width-128 configuration",
...
def verify_manifest(dataset: str, descriptor: dict) -> str:
    ...
    path = result_root(dataset) / "source_manifest.json"
    if path.exists():
...
def initial_audit(model, canonical_sha, bundle, seed: int, arm: str,
    ...
    if random.getstate() != python_rng or hash_rng_state(np.random.get_state()) != hash_rng_state(numpy_rng):
        raise RuntimeError("Initial forward consumed Python or NumPy RNG")
    ...
    audit = {
        ...
        "python_rng_sha256": hash_rng_state(python_rng),
        "numpy_rng_sha256": hash_rng_state(numpy_rng),
    ...
    if arm != "tied":
        paired = result_root(dataset_arg) / f"seed{seed}" / "tied"
        previous = json.loads((paired / "initialization.json").read_text())
        for key in ("canonical_tied_state_sha256", "python_rng_sha256",
...
def train_one(dataset: str, bundle, source_sha: str, seed: int,
    ...
    final = result_root(dataset) / f"seed{seed}" / arm
    if final.exists():
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if not (math.isfinite(val_acc) and math.isfinite(val_ce)):
            raise RuntimeError(f"Nonfinite validation metric at epoch {epoch}")
        ...
        if improved:
            ...
            torch.save({"state_dict": model.state_dict(), "epoch": epoch,
                        "seed": seed, "arm": arm, "mask": MASK, "depth": DEPTH},
                       work / "checkpoint.pt")
    ...
    if (selected["epoch"] != best_epoch or selected["seed"] != seed or
            selected["arm"] != arm or selected["mask"] != MASK or
    ...
    if not (math.isfinite(test_acc) and math.isfinite(test_ce)):
        raise RuntimeError("Nonfinite selected test metric")
    np.savez_compressed(
    ...
    row = {
        "protocol": PROTOCOL,
        "dataset": dataset, "published_split": MASK, "depth": DEPTH,
...
def main():
    ...
    if args.mask not in (1, 2, 3, 4) or args.depth not in (2, 5):
        parser.error("Frozen extension requires masks 1-4 and depths 2 or 5")
    ...
    if args.smoke:
        for seed in SEEDS:
            ...
            for arm in ARMS:
                ...
                rng = (hash_rng_state(random.getstate()),
                       hash_rng_state(np.random.get_state()),
                       hash_tensor(torch.get_rng_state()),
                       hash_tensor(torch.cuda.get_rng_state(device)))
                model.eval()
                with torch.no_grad():
                ...
                if reference_logits is None:
                    ...
                    if delta > INITIAL_LOGIT_TOL:
                        raise RuntimeError(f"Smoke initial logits differ: {seed} {arm} {delta}")
                    if rng != reference_rng:
                        raise RuntimeError(f"Smoke post-construction RNG differs: {seed} {arm}")
                    if canonical_state != reference_state:
                        raise RuntimeError(f"Smoke canonical initial state differs: {seed} {arm}")
                print("SMOKE", args.dataset, MASK, DEPTH, seed, arm,
                      tuple(out.shape), flush=True)
        return
    if device.type != "cuda":
