...
class UntiedBackboneGNNM(nn.Module):
    ...
    Constructing the four stacks by deepcopy draws no new random numbers. With
    the same seed as GNNM, initial parameters and the subsequent training RNG
    state therefore match. After updates, only the propagation stacks are
    untied; the two BatchEnsemble projectors and output norm remain shared.
    """
