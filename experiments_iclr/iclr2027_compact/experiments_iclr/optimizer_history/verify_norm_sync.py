...
def audit_one(dataset, seed, freeze_sha):
    ...
    require(result["protocol"] == "norm_matched_sync_v1" and
    ...
    for epoch, row in enumerate(rows, 1):
        ...
        require(all(math.isfinite(x) for x in (acc, ce, ref, candidate, applied, error, scale)) and
                0 <= acc <= 1 and ce >= 0 and
                min(ref, candidate, applied, error, scale) >= 0,
                f"Invalid NORM-SYNC trace number: {dataset}/{seed}/{epoch}")
        require(abs(applied - ref) <= max(1e-8, 1e-5 * ref) and
                abs(error - abs(applied - ref) / max(ref, 1e-8)) <= 1e-7,
                f"Applied graph-step norm gate failed: {dataset}/{seed}/{epoch}")
        if ref == 0.0:
            require(scale == 0.0 and row["zero_case"] ==
            ...
            require(candidate > 0 and row["zero_case"] == "none" and
                    abs(scale - ref / candidate) <= 1e-6 * max(1.0, ref / candidate),
                    f"Positive graph-step scaling mismatch: {dataset}/{seed}/{epoch}")
    ...
    require(result["selected_epoch"] == best_epoch and
            ...
            result["max_norm_scale"] == max_scale,
    ...
    require(checkpoint["protocol"] == "norm_matched_sync_v1" and
...
def audit_and_lock():
    ...
    require(tuple(matrix["datasets"]) == DATASETS and
            ...
            matrix["norm_absolute_floor"] == 1e-8,
            "NORM-SYNC matrix differs from independent audit constants")
    cells = {}
    for dataset in DATASETS:
        for seed in SEEDS:
            cells[key(dataset, seed)] = audit_one(dataset, seed, freeze_sha)
    actual = {str(path.parent.relative_to(ROOT / "results_norm_sync"))
              for path in (ROOT / "results_norm_sync").rglob("result.json")}
    ...
    lock = {"protocol": "norm_matched_sync_v1", "freeze_sha256": freeze_sha,
...
def audit_scores(device: torch.device):
    ...
    for dataset in DATASETS:
        ...
        for seed in SEEDS:
            ...
            expected.add(name)
            cell = study.cell_dir(dataset, seed)
            score_dir = ROOT / "scores_norm_sync" / name
            ...
            require(sha(cell / "result.json") == locked["result_sha256"] and
                    ...
                    score["protocol"] == "norm_matched_sync_v1" and
    ...
    actual = {str(path.parent.relative_to(ROOT / "scores_norm_sync"))
              for path in (ROOT / "scores_norm_sync").rglob("score.json")}
    ...
    report = {"protocol": "norm_matched_sync_v1", "freeze_sha256": freeze_sha,
              ...
              "scores": records}
    study.write_json(ROOT / "NORM_SYNC_FINAL_SCORE_AUDIT.json", report)
    print(json.dumps({"fresh_replayed_scores": len(records),
                      "report_sha256": sha(ROOT / "NORM_SYNC_FINAL_SCORE_AUDIT.json")}), flush=True)
