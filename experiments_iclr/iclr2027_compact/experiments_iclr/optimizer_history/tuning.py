"""Prospectively frozen validation-grid study for six two-block SAGE arms.
...
SOURCE_FILES = ("tuning.py", "verify_tuning.py", "models.py", "STUDY_PROTOCOL.md",
                "PRETRAIN_REPAIR.md")
DATA_FILES = {
    "wikics": ("data/wiki_cs/data.json",),
    "actor": (
        "data/actor/out1_node_feature_label.txt",
        "data/actor/out1_graph_edges.txt",
        "data/actor/film_split_0.6_0.2_0.npz",
    ),
    "chameleon_filtered": ("data/chameleon_filtered.npz",),
}
FILTERED_SHA = "bf46f07e1fb5249280447e5fe3100f3e82fc4b93ad1e13ffcfdec924b6ac0bb5"
...
def load_wikics():
    raw = json.loads((ROOT / DATA_FILES["wikics"][0]).read_text())
    if len(raw["train_masks"]) != 20 or len(raw["val_masks"]) != 20:
        raise RuntimeError("WikiCS official mask count changed")
    x = torch.tensor(raw["features"], dtype=torch.float32)
    y = torch.tensor(raw["labels"], dtype=torch.long)
    pairs = [(i, j) for i, targets in enumerate(raw["links"]) for j in targets]
    raw_edge = torch.tensor(pairs, dtype=torch.long).t().contiguous()
    edge = to_undirected(raw_edge, num_nodes=len(y))
    return (x, y, raw_edge, edge, torch.tensor(raw["train_masks"][0], dtype=torch.bool),
            torch.tensor(raw["val_masks"][0], dtype=torch.bool),
            torch.tensor(raw["test_mask"], dtype=torch.bool))


def load_actor():
    base = ROOT / "data/actor"
    rows = [line.split("\t") for line in (base / "out1_node_feature_label.txt").read_text().splitlines()[1:] if line.strip()]
    if any(len(row) != 3 for row in rows):
        raise RuntimeError("Actor node row schema changed")
    n = len(rows)
    if {int(row[0]) for row in rows} != set(range(n)):
        raise RuntimeError("Actor node IDs are not exactly 0..n-1")
    width = max(int(j) for _, cols, _ in rows for j in cols.split(",")) + 1
    x = torch.zeros((n, width), dtype=torch.float32)
    y = torch.empty(n, dtype=torch.long)
    for node, cols, label in rows:
        i = int(node)
        x[i, [int(j) for j in cols.split(",")]] = 1.0
        y[i] = int(label)
    pairs = [[int(v) for v in line.split("\t")] for line in
             (base / "out1_graph_edges.txt").read_text().splitlines()[1:] if line.strip()]
    raw_edge = torch.tensor(pairs, dtype=torch.long).t().contiguous()
    edge = to_undirected(coalesce(raw_edge, num_nodes=n), num_nodes=n)
...
def load_filtered():
    path = ROOT / DATA_FILES["chameleon_filtered"][0]
    if sha256_file(path) != FILTERED_SHA:
        raise RuntimeError("Filtered Chameleon official NPZ hash changed")
    with np.load(path, allow_pickle=False) as raw:
        x = torch.from_numpy(raw["node_features"].copy()).float()
        y = torch.from_numpy(raw["node_labels"].copy()).long()
        raw_edge = torch.from_numpy(raw["edges"].copy()).long().t().contiguous()
        if any(raw[key].shape != (10, len(y)) for key in ("train_masks", "val_masks", "test_masks")):
            raise RuntimeError("Filtered Chameleon official split shapes changed")
        train = torch.from_numpy(raw["train_masks"][0].copy()).bool()
        valid = torch.from_numpy(raw["val_masks"][0].copy()).bool()
        test = torch.from_numpy(raw["test_masks"][0].copy()).bool()
...
def load_graph(dataset: str, device: torch.device, include_test: bool = False):
    loaders = {"cora": load_cora, "wikics": load_wikics,
               "actor": load_actor, "chameleon_filtered": load_filtered}
    ...
    split = {"cora": "Planetoid public", "wikics": "official 0",
             "actor": "Geom-GCN 0", "chameleon_filtered": "Platonov filtered 0"}[dataset]
    descriptor = {
...
def raw_files(dataset: str):
    if dataset == "cora":
        ...
        if not files:
            raise RuntimeError("Cora raw files not downloaded")
        return tuple(files)
...
def expected_freeze() -> dict:
    ...
    return {
        "protocol": "validation_tuning_sensitivity_v1", "source_sha256": source,
        "graphs": graphs, "matrix": {
            ...
            "partial_family_search_cost": "12 configurations per graph versus 6 per individual comparator arm",
            "failure_rule": "nonfinite seed invalidates its entire arm/graph candidate",
...
def preflight(device: torch.device):
    ...
    bundle, _ = load_graph("cora", device)
    gpu = {}
    for arm in ARMS:
        seed_all(0)
...
def score(device: torch.device, dataset_subset: str | None = None):
    ...
    if lock["freeze_sha256"] != freeze_sha or lock["protocol"] != "validation_tuning_sensitivity_v1":
    ...
    if len(lock["cells"]) != len(DATASETS) * len(ARMS) * len(CANDIDATES) * len(SEEDS):
        raise RuntimeError("Selection lock lacks complete 432-cell audit")
