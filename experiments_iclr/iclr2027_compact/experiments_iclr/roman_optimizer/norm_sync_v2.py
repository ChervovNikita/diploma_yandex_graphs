"""Prospective V2 12-cell norm ablation with bounded FP32 scale correction."""
...
NORM_REL_TOL = 1e-5
NORM_ABS_FLOOR = 1e-8
ROUNDING_RADIUS = 0.001
ROUNDING_BISECTIONS = 48
SOURCE_FILES = ("norm_sync_v2.py", "verify_norm_sync_v2.py", "NORM_MATCHED_SYNC_V2_PROTOCOL.md")
...
def expected_freeze() -> dict:
    design = json.loads((ROOT / "NORM_SYNC_V2_DESIGN_FREEZE.json").read_text())
    require(design["protocol"] == "norm_matched_sync_design_v2" and
            design["source_sha256"]["NORM_MATCHED_SYNC_V2_PROTOCOL.md"] ==
            sha(ROOT / "NORM_MATCHED_SYNC_V2_PROTOCOL.md") and
            ...
            design["source_sha256"]["NORM_SYNC_FREEZE.json"] ==
            sha(ROOT / "NORM_SYNC_FREEZE.json") and
            ...
            tuple(design["seeds"]) == SEEDS and
            design["rounding_relative_radius"] == ROUNDING_RADIUS and
            design["rounding_bisection_steps"] == ROUNDING_BISECTIONS and
            design["norm_relative_tolerance"] == NORM_REL_TOL and
            design["norm_absolute_floor"] == NORM_ABS_FLOOR,
            "NORM-SYNC implementation differs from prospective design record")
    return {
        "protocol": "norm_matched_sync_v2",
        "design_freeze_sha256": sha(ROOT / "NORM_SYNC_V2_DESIGN_FREEZE.json"),
        ...
        "matrix": {"datasets": DATASETS, "seeds": SEEDS, "planned_cells": 12,
                   ...
                   "rounding_relative_radius": ROUNDING_RADIUS,
                   "rounding_bisection_steps": ROUNDING_BISECTIONS,
                   ...
                   "applied_rule": "float64 target formation and one float32 cast; if the original norm gate fails, deterministic bounded scalar bisection within ±0.1% of nominal scale; no tolerance relaxation or fallback",
...
def check_freeze() -> str:
    path = ROOT / "NORM_SYNC_V2_FREEZE.json"
...
def cell_dir(dataset: str, seed: int) -> Path:
    ...
    return ROOT / "results_norm_sync_v2" / dataset / f"seed{seed}"
...
@torch.no_grad()
def rounded_norm_target(before: torch.Tensor, candidate_delta: torch.Tensor,
                        reference_norm: float, candidate_norm: float) -> dict:
    """Choose one positive scalar, then form and cast the target state once.

    The norm of rounded FP32 coordinate changes is nondecreasing in a
    positive scalar along a fixed direction. Bisection therefore searches
    only the predeclared interval around the nominal norm-matching scalar.
    """
    if reference_norm == 0.0:
        return {"state": before, "nominal_scale": 0.0, "applied_scale": 0.0,
                "relative_scale_correction": 0.0,
                "rounding_correction_iterations": 0,
                "direct_cast_abs_norm_error": 0.0}
    require(candidate_norm > 0.0, "zero sync update with positive reference norm")
    nominal = reference_norm / candidate_norm
    require(math.isfinite(nominal), "Nonfinite norm matching scale")
    before64, delta64 = before.double(), candidate_delta.double()

    def measure(scale: float) -> tuple[float, torch.Tensor]:
        state = (before64 + delta64 * scale).to(before.dtype)
        require(bool(torch.isfinite(state).all()), "Nonfinite applied graph weights")
        applied_delta = state - before
        norm = float(torch.linalg.vector_norm(applied_delta.double()).item())
        require(math.isfinite(norm), "Nonfinite applied graph update norm")
        return norm, state

    allowed = max(NORM_ABS_FLOOR, NORM_REL_TOL * reference_norm)
    direct_norm, direct_state = measure(nominal)
    direct_error = abs(direct_norm - reference_norm)
    best_key = (direct_error, 0.0, nominal)
    best_state, best_scale = direct_state, nominal
    iterations = 0
    if direct_error > allowed:
        low = nominal * (1.0 - ROUNDING_RADIUS)
        high = nominal * (1.0 + ROUNDING_RADIUS)
        low_norm, low_state = measure(low)
        high_norm, high_state = measure(high)
        require(low_norm <= reference_norm <= high_norm,
                "NORM-SYNC V2 correction bracket failed")
        for scale, norm, state in ((low, low_norm, low_state),
                                   (high, high_norm, high_state)):
            trial = (abs(norm - reference_norm), abs(scale / nominal - 1.0), scale)
            if trial < best_key:
                best_key, best_scale, best_state = trial, scale, state
        for _ in range(ROUNDING_BISECTIONS):
            mid = (low + high) / 2.0
            measured, state = measure(mid)
            trial = (abs(measured - reference_norm), abs(mid / nominal - 1.0), mid)
            if trial < best_key:
                best_key, best_scale, best_state = trial, mid, state
            if measured < reference_norm:
                low = mid
            else:
                high = mid
        iterations = ROUNDING_BISECTIONS
        require(best_key[0] <= allowed,
                "NORM-SYNC V2 correction did not meet norm gate")
    relative_correction = best_scale / nominal - 1.0
    require(abs(relative_correction) <= ROUNDING_RADIUS + 1e-12,
            "NORM-SYNC V2 correction radius exceeded")
    return {"state": best_state, "nominal_scale": nominal,
            "applied_scale": best_scale,
            "relative_scale_correction": relative_correction,
            "rounding_correction_iterations": iterations,
            "direct_cast_abs_norm_error": direct_error}
...
def one_norm_matched_update(model, actual_opt, reference, reference_opt, bundle) -> dict:
    ...
    return {"reference_graph_update_norm": reference_norm,
            ...
            "zero_case": zero_case,
            "nominal_norm_scale": target["nominal_scale"],
            "applied_norm_scale": target["applied_scale"],
            "relative_scale_correction": target["relative_scale_correction"],
            "rounding_correction_iterations":
            target["rounding_correction_iterations"],
            "direct_cast_abs_norm_error": target["direct_cast_abs_norm_error"],
...
def preflight(device: torch.device) -> None:
    ...
    write_json(ROOT / f"NORM_SYNC_V2_PREFLIGHT_{str(device).replace(':', '_').upper()}.json",
               report)
...
def train_one(dataset: str, seed: int, bundle, device, freeze_sha: str) -> None:
    ...
    max_rel_scale_correction, correction_steps = 0.0, 0
    max_direct_cast_abs_error = 0.0
    failure = None
    if device.type == "cuda":
    ...
    for epoch in range(1, EPOCHS + 1):
        try:
            ...
            if str(error) not in (
                ...
                "Nonfinite applied graph update norm",
                "NORM-SYNC V2 correction bracket failed",
                "NORM-SYNC V2 correction did not meet norm gate",
                "NORM-SYNC V2 correction radius exceeded"):
                raise
        ...
        min_scale = min(min_scale, step["applied_norm_scale"])
        max_scale = max(max_scale, step["applied_norm_scale"])
        max_rel_scale_correction = max(
            max_rel_scale_correction, abs(step["relative_scale_correction"]))
        correction_steps += int(step["rounding_correction_iterations"] > 0)
        max_direct_cast_abs_error = max(
            max_direct_cast_abs_error, step["direct_cast_abs_norm_error"])
        ...
        traces.append((epoch, valid_acc, valid_ce, int(improved), 1,
                       ...
                       step["relative_norm_error"],
                       step["nominal_norm_scale"], step["applied_norm_scale"],
                       step["relative_scale_correction"],
                       step["rounding_correction_iterations"],
                       step["direct_cast_abs_norm_error"],
                       step["zero_case"]))
    if device.type == "cuda":
        torch.cuda.synchronize(device)
    ...
    if failure is not None:
        result = {"protocol": "norm_matched_sync_v2", "freeze_sha256": freeze_sha,
    ...
    torch.save({"state_dict": best_state, "protocol": "norm_matched_sync_v2",
    ...
    result = {"protocol": "norm_matched_sync_v2", "freeze_sha256": freeze_sha,
              ...
              "max_abs_relative_scale_correction": max_rel_scale_correction,
              "rounding_corrected_steps": correction_steps,
              "max_direct_cast_abs_norm_error": max_direct_cast_abs_error,
              "failure": None}
    write_json(work / "result.json", result)
    work.rename(final)
...
def score_dataset(dataset: str, device: torch.device) -> None:
    ...
    for seed in SEEDS:
        ...
        out = ROOT / "scores_norm_sync_v2" / dataset / f"seed{seed}"
        ...
        write_json(out / "score.json", {
            "protocol": "norm_matched_sync_v2", "freeze_sha256": freeze_sha,
