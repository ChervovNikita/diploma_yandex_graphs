"""Independent artifact and selected-checkpoint replay for ogbn-arxiv.

Writes completion_audit.json only after all 12 arms on one dataset pass.
...
import ogbn_arxiv_sharing_boundary as study
...
def audit_dataset(dataset: str, device: torch.device):
    ...
    require(seed_dirs == {f"seed{s}" for s in study.SEEDS},
            f"Expected exactly three seed directories, found {seed_dirs}")
    records = []
    for seed in study.SEEDS:
        per_seed = root / f"seed{seed}"
        arms = {p.name for p in per_seed.iterdir() if p.is_dir()}
        require(arms == set(study.ARMS),
                f"Expected exactly four complete arms for seed {seed}: {arms}")
        tied_init = read_json(per_seed / "tied" / "initialization.json")
        for arm in study.ARMS:
            run = per_seed / arm
            row = read_json(run / "result.json")
            require(row["protocol"] == "ogbn_arxiv_sharing_boundary_official_time_v1" and
                    row["dataset"] == dataset and row["published_split"] == "official_time" and
            ...
            model.eval()
            with torch.no_grad():
                initial_logits = study.member_logits(model, bundle)[:, :1024, :].cpu().numpy()
    ...
    paired = {}
    for left, right in (("private_first", "private_last"),
                        ("private_first", "tied"),
                        ("private_last", "tied"),
                        ("tied", "untied_propagation")):
    ...
    result = {
        ...
        "equal_private_parameter_counts_per_seed": all(
            keyed[s, "private_first"]["parameter_count"] ==
            keyed[s, "private_last"]["parameter_count"]
    ...
    require(result["equal_private_parameter_counts_per_seed"],
            "Private-first and private-last parameter counts differ")
