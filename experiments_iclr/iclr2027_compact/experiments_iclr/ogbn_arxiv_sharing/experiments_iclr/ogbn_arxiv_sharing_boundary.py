"""Official ogbn-arxiv sharing-boundary diagnostic.

Runs the official ogbn-arxiv time split with optimizer seeds 0/1/2. The two
equal-parameter partial-sharing arms place one private SAGE block first or last.
Tied and fully untied reference arms use the same training schedule. This
diagnostic was selected after prior results were known. Test labels are scored
only after a validation-selected checkpoint is restored.
"""
...
DATA_FILES = {"ogbn_arxiv": (
    "experiments_iclr/ogbn_arxiv_reference_dataset_manifest.json",
    "experiments_iclr/ogbn_arxiv_reference_run_config.json",
    "data/ogb/ogbn_arxiv_official.zip",
)}
SOURCE_FILES = (
    "experiments_iclr/ogbn_arxiv_sharing_boundary.py",
    "experiments_iclr/ogbn_arxiv_sharing_protocol.md",
    "experiments_iclr/verify_ogbn_arxiv_sharing_boundary.py",
...
class OnePrivateBlock(nn.Module):
    """One SAGE residual block per member, the other block shared."""

    def __init__(self, tied: TABMModel, private_index: int):
        super().__init__()
        if len(tied.residual_modules) != 2 or private_index not in (0, 1):
            raise ValueError("Exactly two residual blocks and one private index required")
        ...
        self.private_blocks = nn.ModuleList(
            [copy.deepcopy(tied.residual_modules[private_index])
    ...
    def forward(self, graph, x, tabm_seed):
        ...
        for index in range(2):
            block = (self.private_blocks[tabm_seed]
                     if index == self.private_index else self.shared_block)
            x = block(graph, x)
        x = self.output_normalization(x)
        return self.output_be_block(x, tabm_seed=tabm_seed).squeeze(1)
...
def make_model(arm: str, input_dim: int, classes: int, device: torch.device):
    ...
    if arm in ("private_first", "private_last"):
        private_index = 0 if arm == "private_first" else 1
        model = OnePrivateBlock(tied, private_index).to(device)
        reference_private = tied.residual_modules[private_index].state_dict()
        reference_shared = tied.residual_modules[1 - private_index].state_dict()
        if not all(torch.equal(reference_shared[k], v)
                   for k, v in model.shared_block.state_dict().items()):
            raise RuntimeError("Shared block differs from the tied initialization")
        for private in model.private_blocks:
            if not all(torch.equal(reference_private[k], v)
                       for k, v in private.state_dict().items()):
                raise RuntimeError("Private block differs from the tied initialization")
        expected = (sum(p.numel() for p in tied.parameters()) +
                    (MEMBERS - 1) * sum(
                        p.numel() for p in tied.residual_modules[private_index].parameters()))
        if sum(p.numel() for p in model.parameters()) != expected:
            raise RuntimeError("Partial-sharing parameter count changed")
        if len({p.untyped_storage().data_ptr() for p in model.parameters()}) != len(list(model.parameters())):
            raise RuntimeError("Partial-sharing parameter storage is aliased")
        restore_common_rng()
        return model, canonical
...
def load_graph(dataset: str, device: torch.device):
    ...
    pilot.set_repo_temp()
    bundle, descriptor = pilot.load_official_ogbn_arxiv(ROOT / "data/ogb", device)
    reference = json.loads((ROOT / "experiments_iclr/ogbn_arxiv_reference_dataset_manifest.json").read_text())
    if descriptor != reference:
        raise RuntimeError("Official OGB data fingerprints differ from archived 300-epoch study")
    expected_config = json.loads((ROOT / "experiments_iclr/ogbn_arxiv_reference_run_config.json").read_text())
    fixed = {"backbone": "SAGE", "layers": DEPTH, "hidden_dim": WIDTH,
             "dropout": DROP, "lr": LR, "weight_decay": 0.0,
             "max_epochs": EPOCHS, "min_epochs": EPOCHS,
             "patience": 20, "eval_every": 1}
    if expected_config["configuration"] != fixed:
        raise RuntimeError("Archived OGB-300 optimization configuration differs")
    bundle.classes = bundle.num_classes
...
def expected_manifest(dataset: str, descriptor: dict) -> dict:
    ...
    return {
        "protocol": "ogbn_arxiv_sharing_boundary_official_time_v1",
        "dataset": dataset,
        "split": "official_time",
...
def initial_audit(model, canonical_sha, bundle, seed: int, arm: str,
    ...
    np.save(work / "initial_logits.npy", logits[:, :1024, :])
    audit = {
        ...
        "initial_logits_shape": list(logits[:, :1024, :].shape),
    ...
    if arm != "tied":
        ...
        max_diff = float(np.max(np.abs(logits[:, :1024, :] - reference)))
...
def train_one(dataset: str, bundle, source_sha: str, seed: int,
    ...
    row = {
        "protocol": "ogbn_arxiv_sharing_boundary_official_time_v1",
        "dataset": dataset, "published_split": "official_time",
...
def main():
    ...
    if args.smoke:
        ...
        if parameter_counts["private_first"] != parameter_counts["private_last"]:
            raise RuntimeError("Private-first and private-last counts differ")
        if not (parameter_counts["tied"] < parameter_counts["private_first"] <
                parameter_counts["untied_propagation"]):
            raise RuntimeError("Sharing-boundary parameter-count order changed")
        print("PARAMETER_COUNTS", json.dumps(parameter_counts, sort_keys=True), flush=True)
        return
    if device.type != "cuda":
