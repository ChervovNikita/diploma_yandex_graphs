...
Prepared before any test scoring. The frozen verifier checks all 216
...
def main():
    ...
    check(lock["freeze_sha256"] == freeze_sha and len(lock["cells"]) == 216,
        ...
        for arm in audit.ARMS:
            ...
            for lr, wd in dict.fromkeys((selected, audit.DEFAULT)):
                for seed in audit.SEEDS:
                    ...
                    ta, tc, tp, tm = audit.independent_metrics(
                    ...
                    with np.load(score / "predictions.npz", allow_pickle=False) as earlier:
                        ...
                        earlier_members = earlier["test_member_logits"]
                    ...
                    member_decisions = int(np.count_nonzero(
                        earlier_members.argmax(axis=-1) != tm.numpy().argmax(axis=-1)))
                    check(np.allclose(earlier_valid, valid_new, rtol=1e-5, atol=1e-5) and
                          ...
                          np.allclose(earlier_members, tm.numpy(), rtol=1e-5, atol=1e-5) and
                          valid_decisions == 0 and test_decisions == 0 and member_decisions == 0,
                    ...
                    rows[key] = {"valid_max_abs_diff": valid_diff,
                                 ...
                                 "test_member_decision_mismatches": member_decisions,
    ...
    report = {"freeze_sha256": freeze_sha,
              ...
              "total_test_member_decision_mismatches": 0,
              "per_checkpoint": rows}
