"""Prospective two-graph confirmation of the frozen six-arm selection grid.
...
def load_planetoid(name: str):
    ...
    formal = {"citeseer": "CiteSeer", "pubmed": "PubMed"}[name]
    data = Planetoid(root=str(ROOT / "data" / name), name=formal, split="public")[0]
...
def load_graph(dataset: str, device: torch.device, include_test: bool = False):
    loaders = {"citeseer": lambda: load_planetoid("citeseer"),
...
def raw_files(dataset: str):
    if dataset not in DATASETS:
        raise ValueError(dataset)
    formal = {"citeseer": "CiteSeer", "pubmed": "PubMed"}[dataset]
    base = ROOT / "data" / dataset / formal / "raw"
...
def expected_freeze() -> dict:
    ...
    return {
        "protocol": "planetoid_two_graph_confirmation_v1", "source_sha256": source,
        ...
        "graphs": graphs, "matrix": {
            ...
            "feature_transform": "nonnegative float32 x divided by max(row sum, 1.0); zero rows remain zero",
            "study_status": "new graphs selected after earlier benchmark outcomes; prospectively frozen before this study's training",
...
def preflight(device: torch.device):
    ...
    bundle, _ = load_graph("citeseer", device)
    gpu = {}
    for arm in ARMS:
        seed_all(0)
...
def score(device: torch.device, dataset_subset: str | None = None):
    ...
    if lock["freeze_sha256"] != freeze_sha or lock["protocol"] != "planetoid_two_graph_confirmation_v1":
