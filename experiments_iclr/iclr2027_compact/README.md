...
These checks cover the 40-cell Roman depth grid, its 12-cell longer-budget repeat, 12 no-loop Roman cells, 24 WikiCS/Actor depth cells, 56 earlier four-arm sharing-position records on four graphs, 48 Cora/legacy-Chameleon depth and position arms, 24 filtered-Chameleon arms, nine 1,000-epoch OGB node runs, and 12 link-prediction runs. Each graph uses the split and seed scope stated in its protocol. Seeds are repeated optimization on one split, not independent graph tasks.
