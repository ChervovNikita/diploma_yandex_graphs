#!/bin/bash
# Сплиты 1..NUM_SPLITS-1 (гиперпараметры из лучшей строки split=0 в CSV). Запускать после run_all_experiments_dual_gpu_alex_grid.sh.
...
if [[ "$NUM_SPLITS" -lt 2 ]]; then
    echo "NUM_SPLITS=$NUM_SPLITS: нет сплитов кроме 0, скрипт нечего запускать."
    exit 0
fi

NUM_MODELS=${#MODELS[@]}
NUM_LAYERS=${#LAYERS[@]}
SPAN=$((NUM_LAYERS * (NUM_SPLITS - 1)))
BASE_PER_DS=$((NUM_MODELS * SPAN))
ENSEMBLE_PER_DS=$((NUM_MODELS * (NUM_SPLITS - 1)))
TABM_PER_DS=$((NUM_MODELS * (NUM_SPLITS - 1)))
TASKS_PER_DS=$((BASE_PER_DS + ENSEMBLE_PER_DS + TABM_PER_DS))

LOCK_FILE="/tmp/run_experiments_$(whoami)_alex_$$.lock"
COUNTER_FILE="/tmp/run_experiments_counter_$(whoami)_alex_$$.txt"
...
run_task() {
    ...
    if [[ $t -lt $BASE_PER_DS ]]; then
        local model_idx=$((t / SPAN))
        local rem=$((t % SPAN))
        local layer_idx=$((rem / (NUM_SPLITS - 1)))
        local split=$((1 + rem % (NUM_SPLITS - 1)))
        ...
        local model_idx=$((ens_t / (NUM_SPLITS - 1)))
        local split=$((1 + ens_t % (NUM_SPLITS - 1)))
        local MODEL="${MODELS[$model_idx]}"
        ...
        local model_idx=$((tabm_t / (NUM_SPLITS - 1)))
        local split=$((1 + tabm_t % (NUM_SPLITS - 1)))
        local MODEL="${MODELS[$model_idx]}"
