...
@torch.no_grad()
def evaluate_tabm(model, data, mask, is_binary):
    model.eval()
    outs = []
    for i in range(TABM_INITS):
        out = model(data, data.x, tabm_seed=i)
...
def train_single(model_name, split_idx, data, train_masks, val_masks, test_masks,
    ...
    for step in range(1, NUM_STEPS + 1):
        ...
        val_metrics = evaluate_tabm(model, data, data.val_mask, is_binary)
    ...
    test_m = evaluate_tabm(model, data, data.test_mask, is_binary)
    train_m = evaluate_tabm(model, data, data.train_mask, is_binary)
