...
def evaluate_tabm(model, data, mask):
    ...
    pred = avg_out[mask].argmax(dim=1)
    y_true = data.y[mask].cpu().numpy()
    y_pred = pred.cpu().numpy()
    acc = (pred == data.y[mask]).float().mean().item()
    loss = F.cross_entropy(avg_out[mask], data.y[mask]).item()
...
def train_single(model_name, split_idx, data, train_masks, val_masks, test_masks,
    ...
    for step in range(1, NUM_STEPS + 1):
        ...
        val_metrics = evaluate_tabm(model, data, data.val_mask)
    ...
    test_m = evaluate_tabm(model, data, data.test_mask)
    train_m = evaluate_tabm(model, data, data.train_mask)
...
def main():
    ...
    save_dir = 'checkpoints_tabm'
    os.makedirs(save_dir, exist_ok=True)

    log_path = 'results_tabm.csv'
    ...
    for split_idx in splits:
        for model_name in args.models:
            tag = f'TABM-{model_name} {NUM_LAYERS}L split={split_idx}'
            print(f'\n{"="*60}\nTraining {tag}\n{"="*60}')
